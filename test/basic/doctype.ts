...
suite('Document Type Declaration:', function() {
  test('SYSTEM dtd from create()', () =>
    __.eq(
      __.xml('root', { sysID: 'hello.dtd' }).ele('node').txt('test').end(),
  ...
  test('Empty dtd from create()', () =>
    __.eq(
      __.xml('root', { sysID: '' }).ele('node').txt('test').end(),
  ...
  test('Replace dtd', () =>
    __.eq(
      __.xml('root').dtd('hello.dtd').up().ele('node').txt('test').dtd('bye.dtd').end(),
